...
if (!$pagina) { 
    ...
    $pagina_sig=$pagina +1; 
		...
		$result = mysql_query("SELECT camp0, idcat, nombre, foto, precio FROM tienda WHERE idcat = '$id' order by camp0 DESC", $dbh);
...
$pagina_f1=$pagina_f + 1; 
		...
		$result = mysql_query("SELECT camp0, idcat, nombre, foto, precio FROM tienda WHERE idcat = '$id' order by camp0 DESC  limit $inicio, $TAMANO_PAGINA", $dbh);
...
<? }?>	
    ...
    <tr>
      ...
      <td valign="top"><table width="70%"  border="0" cellpadding="0" cellspacing="0">
        <tr align="center">
          <td align="left">&nbsp;ver mas  : </td>
          <td align="right">&nbsp;
              <?
if ($pagina_ant > 0){ 
	?>
              <a href="todofotos3.php?pagina=<?echo $pagina_ant;?>&amp;busca=<?echo  $busca;?>&amp;email=<? echo  $email; ?>"> Anterior </a> &nbsp;
              <?
}
...
$txt_criterio ="";
if ($total_paginas > 1){ 
    for ($i=$pagina_i;$i<=$pagina_f;$i++){ 
       if ($pagina == $i) {
          echo $pagina . " "; 
         } 
       else {
			?>
              <a href="todofotos3.php?pagina=<?echo $i;?>&amp;busca=<?echo $busca;?>&amp;criterio=<?echo  $txt_criterio;?>&amp;email=<? echo  $email; ?>"> <? echo $i;?> </a>
              <? 
        } 
    } 
} 
...
if ($pagina_sig != $total_paginas+1){ 
?>
            &nbsp; <a href="mercado3.php?pagina=<?echo $pagina_sig;?>&amp;busca=<?echo  $busca;?>&amp;email=<? echo  $email; ?>"> Siguiente </a>&nbsp;
            <? } ?>          </td>
        </tr>
      </table></td>
    </tr>
